//
// by Kim Hyun-Woo | Modified by aatansen
// 2017-06-22
//
...
function createListItem(parameter) {
	...
	if (isSimple) {
		...
		if (item.url == undefined) {
			...
			var title = hierarchy + "\n" + item.title + "\n" + item.url + "\n" + date;
			if (bmp.options.openInNewTab) {
				$a = '<a href="' + item.url + '" target="_blank" title="' + escapeHtml(title) + '">';
			} else {
	...
	if (item.url == undefined) {
		...
		// Use the new _favicon helper for Manifest V3
		$img = '<img src="' + chrome.runtime.getURL("/_favicon/") + '?pageUrl=' + encodeURIComponent(item.url) + '&size=16" class="result-icon">';
	}
